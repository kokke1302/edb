...
```mermaid
flowchart TD
    subgraph Shell["CommonScreen（ボトムナビ共通部）"]
        Translate["/translate<br/>翻訳モード画面"]
        Book["/words<br/>単語帳モード画面"]
    end
 
    Translate <-->|ボトムナビ切替| Book
 
    Shell -->|Drawerを開く| Drawer["MyDrawer<br/>（英文履歴一覧）"]
    Drawer -->|履歴タイルをタップ| Translate
    Drawer -->|設定| Setting["/setting<br/>設定画面"]
    Drawer -->|ヘルプ| Help["/help<br/>ヘルプ画面"]
 
    Translate -->|単語ブロックをタップ| Dict["辞書機能シート<br/>（モーダル）"]
    Dict -->|オリジナルを登録<br/>／候補を編集| Register["/registration<br/>登録画面"]
 
    Book -->|FAB: 新規登録| Register
    Book -->|カードをタップ: 編集| Register
 
    Register -->|保存・キャンセル・削除| Back["元の画面へ戻る"]
```

- `/translate`・`/words`は`ShellRoute`配下のボトムナビで切り替わる（スタックを積まない遷移）。
...
- **英文履歴リスト**: 保存済みの英文をタイル形式で縦に一覧表示する。タップすると該当の翻訳状態を復元して翻訳画面へ遷移する。
