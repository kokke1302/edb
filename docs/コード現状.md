...
本ドキュメントは、開発中アプリプロジェクト内のオブジェクトとデータ構造、その役割を一覧化している。

前半（[アーキテクチャの概要](#アーキテクチャの概要)〜[ディレクトリ構造](#ディレクトリ構造)）は、人間の読者向けに、設計方針とルールをまとめている。

後半（[ファイルの責務](#ファイルの責務)）は、AIによる開発支援を想定し、各ファイル・クラス・メソッドを網羅的に記述している。
...
ただし例外として、ProviderはRepositoryAbstractファイル内で定義している。
...
- **RepositoryImpl**: DBアクセスを含む実装の検証。AppDatabaseをmockして動作を確認する。
...
- `fetch_bookdata_usecase.dart`
  ...
  - `FetchBookDataUseCase`
    - 依存: `BookRepository`
    - `Future<List<CardData>>` execute({required int currentCount, required int pageSize, required SortingData sorter}): `currentCount`を`offset`、`pageSize`を`limit`としてページング・ソート条件で単語リストを取得し、`VocabEntry` → `CardData` に変換して返す
...
- `delete_register_usecase.dart`
  ...
  - `DeleteRegisterUseCase`
    - 依存: `RegisterRepository`
    - `Future<TokenData>` execute({required CardData card, required TokenData token}): DBから単語を削除し、`vocabId`をリセットした `TokenData` を返す
